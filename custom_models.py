...
from typing import Any, Dict, Iterable, List, Optional, Sequence, AsyncIterator
import asyncio
import threading
import torch
...
class Qwen25VLMultiModal(CustomLLM):
    ...
    def metadata(self) -> LLMMetadata:
        return LLMMetadata(
            ...
            context_window=8192,               # concrete integer to avoid validation issues
            num_output=self.max_new_tokens,    # keep in sync with generation config
            is_chat_model=True,
    ...
    def complete(
        ...
        **kwargs: Any,
        ...
        # Apply stop tokens locally for consistency
        if stop:
            for s in stop:
                if s and s in text:
                    text = text.split(s, 1)
    ...
    # ---- async bridges (thread off sync impls) ----
    ...
    async def astream_complete(self, prompt: str, **kwargs: Any) -> AsyncIterator[CompletionResponse]:
    ...
    async def astream_chat(self, messages: List[Any], **kwargs: Any) -> AsyncIterator[ChatResponse]:
        loop = asyncio.get_event_loop()
